...
class TestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def chromosomeLength(self, params):
        ...
        html = env.get_template("length.html").render(
            species=species,
            chromo=chromo,
            length=length
        )
    ...
    def geneLookup(self, params):
        ...
        html = env.get_template("geneLookup.html").render(
            gene=gene,
            gene_id=gene_id
        )
    ...
    def geneSeq(self, params):
        ...
        if gene_id != "Not Found":
            ...
            seq_data = self.get_ensembl_data(
                f"/sequence/id/{gene_id}"
            )
        ...
        html = env.get_template("geneSeq.html").render(
            gene=gene,
            gene_id=gene_id,
            sequence=sequence
        )
    ...
    def geneInfo(self, params):
        ...
        html = env.get_template("geneInfo.html").render(
            gene=gene,
            info=info
        )
    ...
    def geneCalc(self, params):
        ...
        html = env.get_template("geneCalc.html").render(
            gene=gene,
            gene_id=gene_id,
            length=length,
            a=a_pct,
    ...
    def geneList(self, params):
        ...
        html = env.get_template("geneList.html").render(
            chromo=chromo,
            start=start,
            end=end,
            genes=genes
